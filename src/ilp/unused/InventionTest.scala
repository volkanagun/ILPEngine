object InventionTest =
  ...
  engine
    val def testUnion(): Unit = = Parser.parseRule("f(X,Y) :- father(P,X) & mother(Y,P).").get
    val rule1 = Parser.parseRule("t(X,Y) :- f(X,Y) & mother(X,Y).").get
    val rule2 = Invention.union(rule1, rule2)
    inventions.foreach(println(_))

  inventions
    val def my_print(q:Rule): Unit = = Hypothesis(q.getHead(), q)
  ...
  r
    val def my_print(h:Hypothesis): Unit = = engine.ig(database, h)
    println(h)
    println(s"Pos score: ${r.posRate} Neg rate: ${r.negRate}")
    println("=================================================")
  ...
  r
    ...
    val def testIMDB(): Unit = = Parser.parseRule("p(X,Y) :- movie(Z, X) & movie(Z,Y).").get
