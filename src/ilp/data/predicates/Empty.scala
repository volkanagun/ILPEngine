...
class Empty(val nm:String, val variable: Variable) extends Predicate(nm, Array(variable)) {

  override def isDefinite(): Boolean = true
  override def isExecutable(): Boolean = variable.isList() && variable.asNumList().getSize() == 0
